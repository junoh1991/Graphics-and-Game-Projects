...
void Ground::Init(const std::vector<std::string> &search_path) {
    ...
    const int nx = 150; //150
    const int ny = 150; //150
...
bool Ground::ScreenPtHitsPlane(const Matrix4 &view_matrix, const Matrix4 &proj_matrix,
                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    Vector3 normal;
    Point3 point3d_1, point3d_2;

    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d[0], &point3d_1);
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d[stroke2d.size() -1], &point3d_2);
    normal = (point3d_1 - point3d_2).Cross(Vector3(0,1,0));
    // Make sure the normal points to the camera, not away from the camera.
    ...
    Point3 origin;
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d[0], &origin);
    for (int i = 0; i < stroke2d.size(); i ++){
        Point3 temp;

        ScreenPtHitsPlane(view_matrix, proj_matrix, origin, normal, stroke2d[i], &temp);

        stroke3d.push_back(temp);
    }
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
        ...
        Point3 closest_pt_in_plane = P.ClosestPointOnPlane(origin, normal) ;

        float dist = P.DistanceToPlane(origin, normal);
        float h = hfunc(normal, stroke3d, closest_pt_in_plane); 
        if (h != 0){
             Py = (1 - wd(dist)) * Py + wd(dist) * h;
        }
