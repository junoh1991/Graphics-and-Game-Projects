                       ...
                       const Mesh &stroke2d_mesh, const Color &stroke_color)
{
    ...
    SkyStroke tempStroke;
    Mesh m = stroke2d_mesh;

    tempStroke.mesh = m;
    std::vector<Point3> vertices3d;


    for (int i = 0; i < m.num_vertices(); i++){
        Point3 temp_point;
        ScreenPtHitsSky(view_matrix, proj_matrix, Point2(m.vertex(i).x(), m.vertex(i).y()), &temp_point);
        vertices3d.push_back(temp_point);
    }

    tempStroke.mesh.SetVertices(vertices3d);
    tempStroke.color = stroke_color; 

    strokes_.push_back(tempStroke);
}
