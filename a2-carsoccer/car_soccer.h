...
private:
    ...
    Car car_;
    Ball ball_;
    Vector3 gravity_ = Vector3(0, -30, 0);
    float time_;
    const float MAX_SPEED = 75;
    bool fp_view = false;
    
    // Vector used for drawing goal post and boundary box
    std::vector<Point3> goalPost;
    std::vector<Point3> lBound;
    std::vector<Point3> wBound;  
